...
class GREYActions {
  ...
  static actionForPinchFastInDirectionWithAngle(pinchDirection, angle) {
    if (!['outward', 'inward'].some(option => option === pinchDirection))
      throw new Error('pinchDirection should be one of [outward, inward], but got ' + pinchDirection);
    return {
      target: {
        type: 'Class',
        value: 'GREYActions'
  ...
  /*Returns an action that selects @c value on the given @c column of a UIPickerView.
   ...
   @return A GREYAction to set the value of a specified column of a UIPickerView.*/
  static actionForSetDatePickerDateIOSOnly(dateString, dateFormat) {
    return {
      target: {
        type: 'Class',
        value: 'GREYActions'
